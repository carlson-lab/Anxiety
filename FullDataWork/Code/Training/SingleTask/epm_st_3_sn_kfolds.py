...
intercept_mask = OneHotEncoder().fit_transform(epm_dict["y_mouse_train"].reshape(-1,1)).todense()
sample_groups = OrdinalEncoder().fit_transform(epm_dict["y_mouse_train"].reshape(-1,1))
...
s = model.project(epm_dict["X_train"])
X_recon = model.reconstruct(epm_dict["X_train"])
