...
intercept_mask = OneHotEncoder().fit_transform(flx_dict["y_mouse_train"].reshape(-1,1)).todense()
sample_groups = OrdinalEncoder().fit_transform(flx_dict["y_mouse_train"].reshape(-1,1))
...
s = model.project(flx_dict["X_train"])
X_recon = model.reconstruct(flx_dict["X_train"])
