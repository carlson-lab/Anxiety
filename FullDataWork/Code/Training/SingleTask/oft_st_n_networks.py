...
intercept_mask = OneHotEncoder().fit_transform(oft_y_mouse_train.reshape(-1,1)).todense()
sample_groups = OrdinalEncoder().fit_transform(oft_y_mouse_train.reshape(-1,1))
...
model.fit(oft_X_train,
          ...
          verbose=True,
          X_val=oft_X_val,
          y_val=oft_y_task_val.reshape(-1,1),
          best_model_name=saved_model_name)
          
torch.save(model,saved_model_path + saved_model_name)

flx_val_auc = model.score(flx_X,
                          flx_y.reshape(-1,1),
                          flx_y_mouse,
                          return_dict=True)

flx_val_mse = np.mean((flx_X - model.reconstruct(flx_X))**2)
...
oft_val_auc = model.score(oft_X_val,
                            ...
                            oft_y_mouse_val,
                            return_dict=True)
oft_val_mse = np.mean((oft_X_val - model.reconstruct(oft_X_val))**2)
