...
saved_model_name = "long_train_rebal_3_net_{}_model.pt".format(fold)

results_path = "/hpc/home/mk423/Anxiety/FullDataWork/Validations/"
results_file = results_path + "long_train_rebal_3_net_{}_results.pkl".format(fold)
...
model.fit(mt_X_train,
          ...
          verbose=True,
          X_val=mt_X_val,
          y_val=mt_y_val_3_net,
          best_model_name=saved_model_name)

torch.save(model,saved_model_path + saved_model_name)

#Multitask Performance
mt_train_auc = model.score(mt_X_train,mt_y_train_3_net)
mt_val_auc = model.score(mt_X_val,mt_y_val_3_net)

#FLX Performance

flx_y_train = np.hstack([flx_dict["y_train"].reshape(-1,1),
                         flx_dict["y_train"].reshape(-1,1),
                        flx_dict["y_train"].reshape(-1,1)])
flx_y_val = np.hstack([flx_dict["y_val"].reshape(-1,1),
                       flx_dict["y_val"].reshape(-1,1),
                      flx_dict["y_val"].reshape(-1,1)])
...
#EPM Performance
epm_y_train = np.hstack([epm_dict["y_train"].reshape(-1,1),
                         epm_dict["y_train"].reshape(-1,1),
                        epm_dict["y_train"].reshape(-1,1)])
epm_y_val = np.hstack([epm_dict["y_val"].reshape(-1,1),
                       epm_dict["y_val"].reshape(-1,1),
                      epm_dict["y_val"].reshape(-1,1)])
...
#OFT Performance
oft_y_train = np.hstack([oft_dict["y_train"].reshape(-1,1),
                         oft_dict["y_train"].reshape(-1,1),
                        oft_dict["y_train"].reshape(-1,1)])

oft_y_val = np.hstack([oft_dict["y_val"].reshape(-1,1),
                       oft_dict["y_val"].reshape(-1,1),
                      oft_dict["y_val"].reshape(-1,1)])
...
rec_psd_3 = net_3_recon_contribution[:len(anxInfo["powerFeatures"])]
rec_coh_3 = net_3_recon_contribution[len(anxInfo["powerFeatures"]):(len(anxInfo["powerFeatures"]) + len(anxInfo["cohFeatures"]))]
rec_3d_3 = reshapeData(rec_psd_3,rec_coh_3,8,56,anxInfo["powerFeatures"],anxInfo["cohFeatures"],anxInfo["area"])

circle_plot(rec_3d,anxInfo["area"],freqs=np.arange(56),freq_ticks=np.arange(0,56,5),
            min_max_quantiles=(0.85,0.9999),fn=plots_path + "long_train_bal_3_network_net_1_30_component_kf_fold_{}_electome.png".format(fold))

circle_plot(rec_3d_2,anxInfo["area"],freqs=np.arange(56),freq_ticks=np.arange(0,56,5),
            min_max_quantiles=(0.85,0.9999),fn=plots_path + "long_train_bal_3_network_net_2_30_component_kf_fold_{}_electome.png".format(fold))

circle_plot(rec_3d_3,anxInfo["area"],freqs=np.arange(56),freq_ticks=np.arange(0,56,5),
            min_max_quantiles=(0.85,0.9999),fn=plots_path + "long_train_bal_3_network_net_3_30_component_kf_fold_{}_electome.png".format(fold))
...
umc_dt.makeUpperTriangularPlot_pow_coh_gc(net_1_recon_contribution.reshape(1,-1),anxInfo["area"],anxInfo["powerFeatures"],
                                          ...
                                          saveFile=plots_path + "long_train_bal_3_network_net_1_30_component_kf_fold_{}_up-tri.png".format(fold))
umc_dt.makeUpperTriangularPlot_pow_coh_gc(net_2_recon_contribution.reshape(1,-1),anxInfo["area"],anxInfo["powerFeatures"],
                                          anxInfo["cohFeatures"],anxInfo["gcFeatures"],
                                          saveFile=plots_path + "long_train_bal_3_network_net_2_30_component_kf_fold_{}_up-tri.png".format(fold))
umc_dt.makeUpperTriangularPlot_pow_coh_gc(net_3_recon_contribution.reshape(1,-1),anxInfo["area"],anxInfo["powerFeatures"],
                                          anxInfo["cohFeatures"],anxInfo["gcFeatures"],
                                          saveFile=plots_path + "long_train_bal_3_network_net_3_30_component_kf_fold_{}_up-tri.png".format(fold))
