...
saved_model_name = "20_sup_net_fold_{}_model.pt".format(fold)

results_path = "/hpc/home/mk423/Anxiety/FullDataWork/Validations/MultiTask/"
results_file = results_path + "20_sup_net_fold_{}_results.pkl".format(fold)
...
plot_name = plots_path + "sup_net_{}_of_20_fold_{}_{}.png"
...
perc_recon_list = []
for i in range(20):
    X_sup_recon = model.get_comp_recon(torch.Tensor(s).to("cuda"),i) + EPSILON
    net_recon_contribution = np.mean(X_sup_recon/X_recon,axis=0) + EPSILON
    ...
    circle_plot(rec_3d,anxInfo["area"],freqs=np.arange(56),freq_ticks=np.arange(0,56,5),
                min_max_quantiles=(0.85,0.9999),fn=plot_name.format(i,fold,"electome"))
    ...
    umc_dt.makeUpperTriangularPlot_pow_coh_gc(net_recon_contribution.reshape(1,-1),anxInfo["area"],anxInfo["powerFeatures"],
                                              ...
                                              saveFile=plot_name.format(i,fold,"up-tri"))
