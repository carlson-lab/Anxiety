...
class DcsfaNmf(NmfBase):
    ...
    def _initialize(self, dim_in):
        ...
        if self.sup_type=="mc":
            ...
            self.classifier = nn.Sequential(
                nn.Linear(self.n_sup_networks,1),
                nn.Sigmoid()
            )
    ...
    def get_bal_nmf_idxs(self,nmf_groups,y):
        ...
        for group in np.unique(nmf_groups):
            ...
            sub_idxs_pos = np.random.choice(np.arange(np.sum(pos_group_idxs)),
                                            min_samps,
                                            replace=False)
            sub_idxs_neg = np.random.choice(np.arange(np.sum(neg_group_idxs)),
                                            min_samps,
                                            replace=False)
    ...
    def get_balanced_bootstrap(self,X,y,nmf_groups):
        ...
        for group,count in zip(groups,counts):
            if count < max_count:
                ...
                bootstrap_idxs = np.random.choice(np.arange(X_group.shape[0]),
                                                   size=sample_diff,
                                                   replace=True)
    ...
    def get_all_class_predictions(self, X, s, intercept_mask, avg_intercept):
        ...
        if self.sup_type == "mc":
            ...
            y_pred_proba = self.classifier(s[:,:self.n_sup_networks].view(-1,self.n_sup_networks)).view(-1,1)
    ...
    def fit(
        ...
        if nmf_groups is not None:
            if bootstrap:
                X_nmf_pretrain, y_nmf_pretrain = self.get_balanced_bootstrap(X,y_nmf,nmf_groups)
            else:
        ...
        # Pretrain the model.
        if pretrain:
            self.pretrain_NMF(X_nmf_pretrain,
                              y_nmf_pretrain,
                              nmf_max_iter)
    ...
    def score(self, X, y, groups=None, return_dict=False):
        ...
        if groups is not None:
                ...
                if self.sup_type=="mc":
                    for sup_net in range(self.n_sup_networks):
                        ...
                        if len(np.unique(y[group_mask==1,sup_net])) < 2:
                            ...
                            auc = roc_auc_score(y[group_mask==1, sup_net], y_pred[group_mask==1, sup_net])
                        auc_list.append(auc)
                else:
                        if len(np.unique(y[group_mask==1])) < 2:
                            auc = np.nan
                        else:
                            auc = roc_auc_score(y[group_mask==1], y_pred[group_mask==1])
                        auc_list.append(auc)
            ...
            if self.sup_type=="mc":
                ...
                score_results = roc_auc_score(y,y_pred)
